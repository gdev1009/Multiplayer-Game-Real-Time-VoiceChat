-- =============================================================================
-- 0019 — Return graded secret on guess so clients can re-sync YOUR WORD
-- =============================================================================
...
begin
  ...
  if v_used >= s.max_exchanges then
    ...
    where game_id = p_game;
    return jsonb_build_object(
      'ok', true, 'correct', false, 'revealed', true,
  ...
  where game_id = p_game;
  return jsonb_build_object(
    'ok', true, 'correct', false, 'word', v_word, 'word_index', s.word_index
  );
end;
$$;

-- Peek the current secret for the on-the-clock clue-giver (or host driving AI).
create or replace function public.mw_peek_secret(p_game uuid)
returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  ...
  select * into s from public.game_state where game_id = p_game;
  if not found then
    return jsonb_build_object('ok', false, 'reason', 'not_found');
  end if;
  if s.phase not in ('first_half', 'second_half') then
    return jsonb_build_object('ok', false, 'reason', 'not_in_play');
  end if;

  v_role := public.mw_clue_giver_role(s.cluing_team, s.phase);
  if not public.mw_actor_ok(p_game, v_role) then
    return jsonb_build_object('ok', false, 'reason', 'not_clue_giver');
  end if;
  ...
  return jsonb_build_object(
    'ok', true, 'word', v_word, 'word_index', s.word_index
  );
end;
$$;

grant execute on function public.mw_peek_secret(uuid) to authenticated;
