...
- **Starter idle animation** (`core/widgets/idle_animation.dart` + named pose
  ...
  **current artist** `body-female` / `body-male` via `tools/generate_idle_poses.py`
  ...
  cycle on the Opening character card through `IdleCharacterPreview`. Source bodies
  in `base/` were **not** replaced. (`generate_base_bodies.py` is now gated —
  artist bodies are the source of truth via `import_character_art.py`.)
